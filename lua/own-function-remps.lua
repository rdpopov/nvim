...
keymap('n','<Leader>f',':call CycleLanguagesUp()<CR>',{noremap = true, silent = true, desc="Switch to next language"})
keymap('n','<Leader>F', ':call ToggleLocalSpelling()<CR>',{noremap = true, silent = true, desc="Toggle spelling"})
keymap('n','<C-s>', ':call OpenTerm(v:false)<cr>',{noremap = true, silent = true, desc="Open a terminal"})
keymap('n','s', '<Plug>NvimSakHihglightInMotion',{noremap = true, silent = true, desc="Highlight in current selecetion"})
...
keymap('n','<Tab>', '',{noremap = true, desc="Trigger completion or continue current, get next"}) -- ??????
